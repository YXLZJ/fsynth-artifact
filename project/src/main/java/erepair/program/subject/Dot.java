...
@SubjectGroup(group = InputFormat.DOT)
public class Dot extends CSubject {

    public Dot() {
        super(Subjects.GRAPHVIZ);
    }
    ...
    @Override
    Path sourceRootPath() {
        return Main.BIN.resolve("subjects").resolve("dot");
    }

    @Override
    Path executablePath() {
        return sourceRootPath().resolve("build").resolve("dot_parser");
    }
