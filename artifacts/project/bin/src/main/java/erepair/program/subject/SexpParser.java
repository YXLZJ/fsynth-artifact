/**
 * The sexp-parser from https://github.com/takagi/sexp-parser - modified to report incomplete inputs
 */
...
@SubjectGroup(group = InputFormat.SEXP)
public class SexpParser extends CSubject {

    public SexpParser() {
        super(Subjects.SEXP_PARSER);
    }


    @Override
    Path sourceRootPath() {
        return Main.BIN.resolve("subjects").resolve("sexp-parser");
    }

    @Override
    Path executablePath() {
        return sourceRootPath().resolve("sexp.cov");
    }
...
//                List.of("./configure", "CFLAGS=-fprofile-arcs -ftest-coverage", "--enable-gcov", "--disable-docs"),
    ...
    String[] acceptedSuffixes() {
        return new String[]{
                SexpParser.class.getAnnotation(SubjectGroup.class).group().getSuffix()
        };
    }
}
